...
export async function POST(req: NextRequest) {
  ...
  const { data: record, error: dbError } = await supabase
    .from("demo_requests")
    .insert({ business_name, niche_key, phone_number, consent_given: true, call_status: "queued", transcript: null, booked_demo: false })
    .select()
    .single();
  ...
  console.log("[vapi] call placed:", { call_id: vapiCall.id, to: phone_number, demo_request_id: record.id });
